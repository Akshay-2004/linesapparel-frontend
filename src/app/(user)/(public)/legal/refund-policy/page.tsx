...
export default function RefundPolicyPage() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const pageData = await getLegalPageByType('refund-policy');
        setPage(pageData);
          ...
          <Card className="shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-lg text-gray-600 mb-6">
                Unable to load the refund policy at this time.
              </p>
              <p className="text-gray-500">
                {error || 'The refund policy document could not be found. Please try again later.'}
              </p>
            </CardContent>
          </Card>
        </div>
