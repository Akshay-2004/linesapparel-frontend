...
export default function ShippingPolicyPage() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const pageData = await getLegalPageByType('shipping-policy');
        setPage(pageData);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading shipping policy...</p>
        </div>
      </div>
    );
  }
          ...
          <Card className="shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-lg text-gray-600 mb-6">
                Unable to load the shipping policy at this time.
              </p>
              <p className="text-gray-500">
                {error || 'The shipping policy document could not be found. Please try again later.'}
              </p>
            </CardContent>
          </Card>
        </div>
